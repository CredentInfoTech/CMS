  ...
  const saveAzureSectionData = async (requestId: number) => {
    ...
    const filteredAzureSectionData =
      props.rowEdit === "Yes"
        ...
        : azureSectionData;
    try {
      for (const row of filteredAzureSectionData) {
        let TotalInvoive;
        // let AreBothChargesReuired;
        const totalInvoiceValue = getInvoiceTotal(row.popupRows || []);
        // const bothChargesRequired = row.bothChargesRequired;
        ...
        // AreBothChargesReuired = bothChargesRequired;
        const cmsInvoiceData = {
          RequestID: requestId,
          // AreBothChaqrgesRequired: AreBothChargesReuired,
        ...
        if (row && row.popupRows && row.popupRows.length > 0) {
          for (const popupRow of row.popupRows) {
            // Attach the parent request ID to each popup row
            const popupData = {
              RequestID: requestId,
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="row d-flex justify-content-end">
            <div className="col-md-6 d-flex justify-content-end">
              ...
              {props.selectedRow &&
              formData.productServiceType.toLowerCase() === "azure" &&
              props.selectedRow.isAzureRequestClosed !== "Yes" && props.selectedRow.employeeEmail === currentUserEmail ? (
                <>
                  <button
                    type="submit"
                    className="btn btn-success w-40 mt-3 "
